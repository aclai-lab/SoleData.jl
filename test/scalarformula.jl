...
@test_nowarn SoleData.RangeScalarCondition(
    parsefeature(VarFeature, "V1"), nothing, 1, true, true,)
@test_broken SoleData.RangeScalarCondition(
    parsefeature(VarFeature, "V1"), nothing, nothing, true, true,)
...
formula = @scalarformula ((V1 > 10) ∧ (V2 < 0) ∧ (V3 > 10)) ∨
                         ((V1 <= 0) ∧ (V2 == 2) ∧ (V3 != 10))
...
my_dnf = x->SoleLogics.dnf(x;
    (;
        ...
        flip_atom = a -> SoleData.polarity(SoleData.test_operator(SoleLogics.value(a))) ==
                         false,
    )...,
)

@test_broken my_dnf(formula1) == my_dnf(formula2) == my_dnf(formula3) == my_dnf(formula4) ==
             my_dnf(formula5)
@test string(my_dnf(formula1)) == string(my_dnf(formula2)) == string(my_dnf(formula3)) ==
      string(my_dnf(formula4)) == string(my_dnf(formula5))
