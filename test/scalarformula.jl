...
my_dnf =
    x->SoleLogics.dnf(
        ...
        (;
            ...
            flip_atom=a ->
                SoleData.polarity(SoleData.test_operator(SoleLogics.value(a))) == false,
        )...,
    )

@test_broken my_dnf(formula1) ==
    my_dnf(formula2) ==
    my_dnf(formula3) ==
    my_dnf(formula4) ==
    my_dnf(formula5)
@test string(my_dnf(formula1)) ==
    string(my_dnf(formula2)) ==
    string(my_dnf(formula3)) ==
    string(my_dnf(formula4)) ==
    string(my_dnf(formula5))
