...
φ = Atom(parsecondition(SoleData.ScalarCondition, "sepal_length > 5.8"; featuretype = SoleData.VariableValue)) ∧
...
φ = parseformula("sepal_length > 5.8 ∧ sepal_width < 3.0 ∨ target == \"setosa\"";
...
@test_nowarn alphabet(X; test_operators = [≥, <]) |> atoms .|>
             x->syntaxstring(x; show_colon = false)
...
@test_broken begin
    ...
    local c = parsecondition(SoleData.ScalarCondition, "x < 0.03425152849651658";
