...
@testset "SoleBase.jl" begin
    ...
    @testset "savedataset" begin
        ...
        @test length(filter((x) -> occursin(_ds_frame_prefix, x), readdir(joinpath(path, _ds_inst_prefix * "1")))) == 2
        @test parse.(Int64, split(filter((row) -> occursin("num_frames", row) , readlines(joinpath(path, _ds_metadata)))[1], "=")[2]) == 2
        @test length(filter((row) -> occursin("frame", row) , readlines(joinpath(path, _ds_metadata)))[2:end]) == 2
        frames = filter((row) -> occursin("frame", row) , readlines(joinpath(path, _ds_metadata)))[2:end]
        @test all([parse.(Int64, split(string(frame), "=")[2]) == dimension(lmfd[i_frame]) for (i_frame, frame) in enumerate(frames)])
        @test parse(Int64, split(filter((row) -> occursin("num_classes", row) , readlines(joinpath(path, _ds_metadata)))[1], "=")[2]) == 1
        @test length(filter((x) -> occursin(_ds_inst_prefix, x), readdir(joinpath(path)))) == 2
        
        # instances Metadata.txt
        @test all([isfile(joinpath(path, _ds_inst_prefix * string(i), _ds_metadata)) for i in 1:nrow(lmfd[1])])
        for nframe in 1:nframes(lmfd)
            for (i, dim_frame) in enumerate(filter((row) -> occursin("dim_frame", row) , readlines(joinpath(path, _ds_inst_prefix * string(nframe), _ds_metadata))))
                @test parse(Int64, split(split(split(dim_frame, "=")[2], ",")[1], "(")[2]) == length(CSV.read(joinpath(path, _ds_inst_prefix * string(nframe), _ds_frame_prefix * string(i)) * ".csv", DataFrame)[:,1])
            end
        end
        @test length([filter((row) -> occursin(string(labels), row) , readlines(joinpath(path, _ds_inst_prefix * string(frame), _ds_metadata))) for labels in labels(lmfd) for frame in 1:nframes(lmfd)]) == 2
        @test [filter((row) -> occursin(string(labels), row) , readlines(joinpath(path, _ds_inst_prefix * string(frame), _ds_metadata))) for labels in labels(lmfd) for frame in 1:nframes(lmfd)] == [["name=Python"],["name=Julia"]]

        # Example
        @test all([isdir(joinpath(path, _ds_inst_prefix * string(instance))) for instance in 1:nrow(lmfd[1])])
        @test all([isfile(joinpath(path, _ds_inst_prefix * string(instance), _ds_frame_prefix * string(i_frame) * ".csv")) for i_frame in 1:length(lmfd) for instance in 1:nrow(lmfd[1])])
