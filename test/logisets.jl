...
@test [
    SoleData.checkcondition(bool_condition, bool_logiset, i_instance, w) for w in worlds
] == Bool[0, 0, 1, 0, 1, 0, 1, 0, 0, 0]
@test_nowarn displaystructure(bool_logiset)

# Scalar (Float)
rng = StableRNG(1)
scalar_logiset = SoleData.ExplicitModalLogiset([(
    Dict([w => Dict([f => rand(rng) for f in features]) for w in worlds]), fr
)])
scalar_condition = SoleData.ScalarCondition(features[1], >, 0.5)
@test [
    SoleData.checkcondition(scalar_condition, scalar_logiset, i_instance, w) for w in worlds
] == Bool[1, 0, 1, 0, 0, 0, 1, 1, 0, 0]
@test_nowarn displaystructure(scalar_logiset)

# Non-scalar (Vector{Float})
rng = StableRNG(2)
nonscalar_logiset = SoleData.ExplicitModalLogiset([(
    Dict([w => Dict([f => rand(rng, rand(rng, 1:3)) for f in features]) for w in worlds]),
    fr,
)])
nonscalar_condition = SoleData.FunctionalCondition(features[1], (vals)->length(vals) >= 2)
@test [
    SoleData.checkcondition(nonscalar_condition, nonscalar_logiset, i_instance, w) for
    w in worlds
...
@test SoleData.modalitytype(multilogiset) <: SoleData.AbstractModalLogiset{
    SoleLogics.World{Int64},
    U,
    SoleData.Feature{String},
    SoleLogics.SimpleModalFrame{SoleLogics.World{Int64},Graphs.SimpleDiGraph{Int64}},
} where {U}
...
@test_nowarn SoleData.AbstractModalLogiset{
    ...
    {
    U<:SoleData.AbstractModalLogiset{
        SoleLogics.World{Int64},
        U,
        Feature{String},
        SoleLogics.SimpleModalFrame{SoleLogics.World{Int64},SimpleDiGraph{Int64}},
    },
} where {U}
...
for w in worlds
    ...
    @test representatives(fr, w, scalar_condition) ==
        representatives(scalar_logiset, 1, w, scalar_condition)
end
...
for w in worlds
    @test (featvalue(features[1], scalar_logiset, 1, w) > 0.9) ==
        check(Atom(cond1) ∧ ⊤, scalar_logiset, 1, w)
    @test (featvalue(features[2], scalar_logiset, 1, w) > 0.3) ==
        check(Atom(cond2) ∧ ⊤, scalar_logiset, 1, w)
end
...
for w in worlds
    @test (
        (featvalue(features[1], scalar_logiset, 1, w) > 0.9) &&
        (featvalue(features[2], scalar_logiset, 1, w) > 0.3)
    ) == check(φ, scalar_logiset, 1, w)
end

# Modal formula
φ = ◊(⊤ → Atom(cond1) ∧ Atom(cond2))
for w in worlds
    @test check(φ, scalar_logiset, 1, w) == (
        length(accessibles(fr, w)) > 0 && any([
            (
                (featvalue(features[1], scalar_logiset, 1, v) > 0.9) &&
                (featvalue(features[2], scalar_logiset, 1, v) > 0.3)
...
for w in worlds
    @test check(φ, multilogiset, 2, 1, w) == (
        length(accessibles(fr, w)) > 0 && any([
            (
                (featvalue(features[1], multilogiset, 2, 1, v) > 0.9) &&
                (featvalue(features[2], multilogiset, 2, 1, v) > 0.3)
            ) for v in accessibles(fr, w)
        ])
    )
end
...
@test SoleData.featvalue(features[1], nonscalar_logiset, 1, worlds[1]) ==
    SoleData.featvalue(features[1], nonscalar_supported_logiset, 1, worlds[1])
...
@test SoleLogics.allworlds(bool_logiset, 1) ==
    SoleLogics.allworlds(bool_supported_logiset, 1)
...
@test_nowarn SoleData.parsecondition(
    SoleData.ScalarCondition, "p > 0.5"; featvaltype=String, featuretype=Feature
)
@test SoleData.ScalarCondition(features[1], >, 0.5) == SoleData.parsecondition(
    SoleData.ScalarCondition, "p > 0.5"; featvaltype=String, featuretype=Feature
)
...
memoset = [
    ThreadSafeDict{SyntaxTree,Worlds{W}}() for
    i_instance in 1:ninstances(bool_supported_logiset)
]
...
rng = StableRNG(1)
alph = ExplicitAlphabet([
    SoleData.ScalarCondition(rand(rng, features), rand(rng, [>, <]), rand(rng)) for
    i in 1:10
])
syntaxstring.(alph)
_formulas = [
...
c5 = @test_nowarn [
    check(φ, SupportedLogiset(bool_logiset), 1, w; use_memo=nothing) for φ in _formulas
]
...
memoset = [
    ThreadSafeDict{SyntaxTree,Worlds{W}}() for i_instance in 1:ninstances(scalar_logiset)
]
