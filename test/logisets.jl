...
φ = ◊(⊤ → Atom(cond1) ∧ Atom(cond2))
for w in worlds
    @test check(φ, scalar_logiset, 1, w) == (length(accessibles(fr, w)) > 0 &&
           any([((featvalue(features[1], scalar_logiset, 1, v) > 0.9) &&
...
for w in worlds
    @test check(φ, multilogiset, 2, 1, w) == (length(accessibles(fr, w)) > 0 &&
           any([((featvalue(features[1], multilogiset, 2, 1, v) > 0.9) &&
...
@test SoleData.ScalarCondition(features[1], >, 0.5) ==
      SoleData.parsecondition(SoleData.ScalarCondition, "p > 0.5"; featvaltype = String, featuretype = Feature)

############################################################################################
# Memoset's
############################################################################################

memoset = [ThreadSafeDict{SyntaxTree, Worlds{W}}()
           for i_instance in 1:ninstances(bool_supported_logiset)]
...
rng = StableRNG(1)
alph = ExplicitAlphabet([SoleData.ScalarCondition(rand(rng, features), rand(rng, [>, <]), rand(rng))
                         for i in 1:10])
syntaxstring.(alph)
_formulas = [randformula(
                 rng, 4, alph, [NEGATION, CONJUNCTION, IMPLICATION, DIAMOND, BOX];
                 mode = :exactheight,)
             for i in 1:10]
...
c5 = @test_nowarn [check(φ, SupportedLogiset(bool_logiset), 1, w; use_memo = nothing)
                   for φ in _formulas]
...
memoset = [ThreadSafeDict{SyntaxTree, Worlds{W}}()
           for i_instance in 1:ninstances(scalar_logiset)]
