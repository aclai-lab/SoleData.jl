...
@test !SoleData.ismultilogiseed(Vector{Array{Float32, 3}}())

n_instances = 2
_nvars = 2

multidataset,
multirelations = collect.(zip([
...
multilogiset = @test_logs min_level=Logging.Error scalarlogiset(
    multidataset; use_full_memoization = false, use_onestep_memoization = true,
    conditions = metaconditions, relations = multirelations,)
multilogiset = @test_logs min_level=Logging.Error scalarlogiset(
    multidataset; use_full_memoization = true, use_onestep_memoization = true,
    conditions = metaconditions, relations = multirelations,)

metaconditions = vcat([[ScalarMetaCondition(feature, >), ScalarMetaCondition(feature, <)]
                       for feature in generic_features]...)
complete_supported_multilogiset = @test_logs min_level=Logging.Error scalarlogiset(
    multidataset; use_full_memoization = true, use_onestep_memoization = true,
    conditions = metaconditions, relations = multirelations,)
...
alph = ExplicitAlphabet([SoleData.ScalarCondition(
                             rand(rng, generic_features), rand(rng, [
                                 >, <,]), rand(rng),) for i in 1:n_instances]);
# syntaxstring.(alph)
                         ...
                         f = randformula(rng,
                             ...
                             [SoleLogics.BASE_PROPOSITIONAL_CONNECTIVES...,
                                 vcat([[DiamondRelationalConnective(r),
                                           BoxRelationalConnective(r),] for r in relations]...)[1:32:end]...,],)
                         if rand(Bool)
                             coin = rand(1:2)
                             f = begin
                                 if coin == 1
                                     ...
                                     # AnchoredToWorldFormula(f, SoleData.WorldCheck(rand(collect(allworlds(modality(multilogiset, i_modality), i_instance)))))
                                     # else
                                     #     f
                                 end
                             end
...
c3 = @test_nowarn [check(φ, complete_supported_multilogiset, i_instance)
                   for φ in multiformulas]
