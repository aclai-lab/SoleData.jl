...
multidataset, multirelations = collect.(
    zip(
        [
            ...
            (
                Array(reshape(1.0:36.0, 3, 3, _nvars, n_instances)),
...
generic_features = collect(
    Iterators.flatten([[VariableMax(i_var), VariableMin(i_var)] for i_var in 1:_nvars])
)
...
alph = ExplicitAlphabet([
    SoleData.ScalarCondition(rand(rng, generic_features), rand(rng, [>, <]), rand(rng)) for
    i in 1:n_instances
]);
# syntaxstring.(alph)
...
@test_throws MethodError checkcondition(
    SoleLogics.value(alph.atoms[1]), multilogiset, i_instance
)

c1 = @test_nowarn [check(φ, multilogiset, i_instance) for φ in multiformulas]
c3 = @test_nowarn [
    check(φ, complete_supported_multilogiset, i_instance) for φ in multiformulas
]
