...
bool_relationalmemoset = @test_nowarn ScalarOneStepRelationalMemoset(
    bool_logiset, metaconditions, [globalrel], perform_initialization,)
bool_globalmemoset = @test_nowarn ScalarOneStepGlobalMemoset(
    bool_logiset, metaconditions, perform_initialization,)
...
bool_onestepmemoset = @test_logs min_level=Logging.Error ScalarOneStepMemoset{Bool}(
    bool_relationalmemoset, bool_globalmemoset, metaconditions, relations,)
...
bool_onestepmemoset_empty = ScalarOneStepMemoset(
    bool_logiset, metaconditions, [globalrel]; precompute_globmemoset = false,
    precompute_relmemoset = false, print_progress = true,)
bool_onestepmemoset_full = ScalarOneStepMemoset(
    bool_logiset, metaconditions, [globalrel]; print_progress = true,)
...
memoset = [ThreadSafeDict{SyntaxTree, Worlds}()
           for i_instance in 1:ninstances(bool_logiset)]
@test_broken bool_logiset_3layer = SupportedLogiset(bool_logiset, [
    bool_onestepmemoset_empty, memoset,])
...
alph = ExplicitAlphabet([SoleData.ScalarCondition(rand(rng, features), rand(rng, [>, <]), rand(rng))
                         for i in 1:10])
syntaxstring.(alph)
_formulas = [randformula(rng, 10, alph, SoleLogics.BASE_MULTIMODAL_CONNECTIVES)
             for i in 1:20];
...
############################################################################################
c1 = @test_nowarn [check(φ, bool_logiset, 1, w; perform_normalization = false)
                   for φ in _formulas]
...
############################################################################################
c2 = @test_nowarn [check(φ, bool_logiset_2layer, 1, w; perform_normalization = false)
                   for φ in _formulas]
