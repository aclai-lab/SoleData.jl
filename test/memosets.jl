...
@test_broken ScalarOneStepMemoset(
...
bool_onestepmemoset_empty = ScalarOneStepMemoset(
    ...
    metaconditions,
    [globalrel];
    precompute_globmemoset=false,
    precompute_relmemoset=false,
    print_progress=true,
)
...
@test_broken bool_logiset_3layer = SupportedLogiset(
...
syntaxstring.(alph)
_formulas = [
    randformula(rng, 10, alph, SoleLogics.BASE_MULTIMODAL_CONNECTIVES) for i in 1:20
];
...
############################################################################################
c2 = @test_nowarn [
    check(φ, bool_logiset_2layer, 1, w; perform_normalization=false) for φ in _formulas
]
...
@test_nowarn concatdatasets(
    bool_relationalmemoset, bool_relationalmemoset, bool_relationalmemoset
)
