...
@test SoleData.readrelations(:IA7, X) == [
    ...
    SoleLogics._IA_AorO(),
    SoleLogics._IA_L(),
    SoleLogics._IA_DorBorE(),
    SoleLogics._IA_AiorOi(),
    SoleLogics._IA_Li(),
    SoleLogics._IA_DiorBiorEi(),
]
...
@test SoleData.readrelations(:RCC8, X) == [
    ...
    SoleLogics._Topo_EC(),
    SoleLogics._Topo_PO(),
    SoleLogics._Topo_TPP(),
    SoleLogics._Topo_TPPi(),
    SoleLogics._Topo_NTPP(),
    SoleLogics._Topo_NTPPi(),
]
@test SoleData.readrelations(:RCC5, X) == [
    ...
    SoleLogics._Topo_PO(),
    SoleLogics._Topo_PP(),
    SoleLogics._Topo_PPi(),
]

@test SoleData.autorelations(:IA7) == (:IA7, "")
@test a=SoleData.autorelations(:invalid) == (
    nothing,
...
mutable struct Fake_MDT_autologiset_test # <: MMI.Probabilistic
    ...
    featvaltype::Type
    relations::Union{Nothing,Symbol,Vector{<:AbstractRelation},Function}
    ...
    function Fake_MDT_autologiset_test(;
        ...
        new(
            downsize, conditions, featvaltype, relations, fixcallablenans, force_i_variables
        )
    end
end
...
X, relations = (
    DataFrame(;
        NamedTuple([
            Symbol(i_var) => [rand(3, 3) for i_instance in 1:n_instances] for
            i_var in 1:_nvars
        ])...,
    ),
...
logiset_none = scalarlogiset(
    X;
    use_full_memoization=false,
    relations=SoleData.readrelations(:none, X),
    conditions=metaconditions,
)
logiset_IA = scalarlogiset(
    X;
    use_full_memoization=false,
    relations=SoleData.readrelations(:IA, X),
    conditions=metaconditions,
)
logiset_IA7 = scalarlogiset(
    X;
    use_full_memoization=false,
    relations=SoleData.readrelations(:IA7, X),
    conditions=metaconditions,
)
logiset_IA3 = scalarlogiset(
    X;
    use_full_memoization=false,
    relations=SoleData.readrelations(:IA3, X),
    conditions=metaconditions,
)
logiset_RCC8 = scalarlogiset(
    X;
    use_full_memoization=false,
    relations=SoleData.readrelations(:RCC8, X),
    conditions=metaconditions,
)
logiset_RCC5 = scalarlogiset(
    X;
    use_full_memoization=false,
    relations=SoleData.readrelations(:RCC5, X),
    conditions=metaconditions,
)
