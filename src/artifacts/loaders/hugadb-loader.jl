...
function _load_hugadb(
    ...
    X = DataFrame(
        [
         ...
         for i in 1:length(_variablenames)], _variablenames,)
...
function load(l::HuGaDBLoader)
    ...
    X, (activity_strings, activity_ids),
    _ = _load_hugadb(
        dirpath,
        expfiles(l)[1],
    ...
    return vcat([X,
        [_load_hugadb(dirpath, filename; _variablenames = variablenames(l)) |> first
         for filename in expfiles(l)[2:end]]...,]...), (activity_strings, activity_ids),
    variablenames(l)
end
...
function filter_hugadb(X::DataFrame, id; labelcolumn::Integer = 39)
    ...
    _X = [let indices = findall(x -> x == id, X[instance, labelcolumn])
              isempty(indices) ? nothing :
              DataFrame(
                  [[X[instance, variable][indices]]
                   for variable in 1:nvariables], variablenames,)
          end
          for instance in 1:(X |> size |> first)] |> x -> filter(!isnothing, x)
