...
function load_arff_dataset(
    ...
    (X_test,
        ...
        if (any(startswith.(path, ["https://", "http://"])))
            ...
            # )
            (
                read(z.files[[f.name == "$(dataset_name)_TRAIN.arff" for f in z.files]][1], String) |>
                parseARFF,
                read(z.files[[f.name == "$(dataset_name)_TEST.arff" for f in z.files]][1], String) |>
                parseARFF,
            )
        else
            (
...
```
"""
function load_dataset(dataset_name::AbstractString)::Tuple{
        <:AbstractDataFrame, <:CategoricalArray,}
