...
function scalar_simplification(φ::DNF, args...; kwargs...)
    return map(d->scalar_simplification(d, args...; kwargs...), SoleLogics.disjuncts(φ)) |>
           LeftmostDisjunctiveForm
end
function scalar_simplification(φ::CNF, args...; kwargs...)
    return map(d->scalar_simplification(d, args...; kwargs...), SoleLogics.conjuncts(φ)) |>
...
function scalar_simplification(
    ...
    φ = LeftmostLinearForm(SoleLogics.connective(φ),
        map(ch->begin
                if ch isa Atom
                    ch
    ...
    if !all(c->c isa Atom{<:Union{ScalarCondition, RangeScalarCondition}}, SoleLogics.grandchildren(φ))
        ...
        !silent &&
            println("Cannot perform scalar simplification on linear form:\n$(syntaxstring(φ))\n on" *
    ...
    ch = collect(Iterators.flatten([begin
                                        ...
                                        conds = [if cond isa ScalarCondition &&
                                                    (test_operator(cond) == (==))
                                        ...
                                        conds = Iterators.flatten([if cond isa
                                                                      ScalarCondition
                                                                       [cond]
                                                                   elseif cond isa
                                                                          RangeScalarCondition
                                        ...
                                        for cond in conds
                                            ...
                                            this_domain = (
                                                test_operator(cond), threshold(cond),)
                                                ...
                                                if isnothing(max_domain) ||
                                                   (
                                                    (isless(this_domain[2], max_domain[2]) ||
                                                     (==(this_domain[2], max_domain[2]) &&
                                                      my_isless(this_domain[1], max_domain[1]))
                                                ) == conn_polarity)
                                                ...
                                                if isnothing(min_domain) ||
                                                   (
                                                    (!(isless(this_domain[2], min_domain[2])) ||
                                                     (==(this_domain[2], min_domain[2]) &&
                                                      my_isless(this_domain[1], min_domain[1]))
                                                ) == conn_polarity)
                                        ...
                                        if !isnothing(max_domain) &&
                                           !isnothing(min_domain) &&
                                           (max_domain[2] < min_domain[2]) # TODO make it more finegrained so that it captures cases with < and >=
                                            nothing
                                        elseif isnothing(min_domain) &&
                                               isnothing(max_domain)
                                            nothing
                                        else
                                            if force_scalar_range_conditions
                                                min_domain = isnothing(min_domain) ?
                                                             (>=, nothing#= typemin(T) =#) :
                                                             min_domain
                                                max_domain = isnothing(max_domain) ?
                                                             (<=, nothing#= typemax(T) =#) :
                                                             max_domain
                                            end
                                            if allow_scalar_range_conditions &&
                                               (!isnothing(min_domain) &&
                                                !isnothing(max_domain))
                                                minincluded = (!SoleData.isstrict(min_domain[1])) ||
                                                              isnothing(min_domain[2])
                                                maxincluded = (!SoleData.isstrict(max_domain[1])) ||
                                                              isnothing(max_domain[2])
                                                push!(out,
                                                    Atom(SoleData.RangeScalarCondition(
                                                        feat, min_domain[2], max_domain[2],
                                    ...
                                    for (feat, bitmask) in feature_groups]))

    # @show SoleLogics.connective(φ)
    ψ = (length(ch) == 0 ? (⊤) :
         (length(ch) == 1 ? first(ch) : LeftmostLinearForm(SoleLogics.connective(φ), ch)))
end
