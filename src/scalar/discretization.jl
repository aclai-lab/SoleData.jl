# Implementation for
# Multi-interval discretization of continuous-valued attributes for classification learning, Fayyad, 1993
...
function _entropy_discretize_sorted(
    ...
    MDLP_Gain_threshold = ((log2(N - 1) / N) + (delta / N))
    ...
    println("N = $(N)")
    println("\t$(Gain) > $(MDLP_Gain_threshold)")
    println("========================================")
    readline()
    ...
    #
    if N > 1 && Gain > MDLP_Gain_threshold
