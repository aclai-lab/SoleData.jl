...
function randatom(
        ...
        metaconditions::Union{
            Nothing, ScalarMetaCondition, AbstractVector{<:ScalarMetaCondition},} = nothing,
    ...
    metaconditions = metaconditions isa ScalarMetaCondition ? [metaconditions] :
                     metaconditions
            ...
            filtered_featconds = filter(
                mc_thresholds->begin
                    ...
                    return (isnothing(features) || feature(mc) in features) &&
                           (isnothing(test_operators) ||
                            test_operator(mc) in test_operators)
                end,
                grouped_featconditions,)
