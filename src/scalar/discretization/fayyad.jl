...
function entropy_normalized2(D)
    ...
    return [
        (pr=pr[pr .!= 0]; -sum(pr .* log.(2, pr))) for pr in rows
    ]
end
...
function _entropy_discretize_sorted(C; force=false)
...
function discretize(varvalue::AbstractVector, y::AbstractVector)
