...
function merge_bounds(vb1::VariableBounds, vb2::VariableBounds)::VariableBounds
    ...
    new_lower, new_has_lower,
    new_lower_inclusive = if !vb1.has_lower && !vb2.has_lower
        (-Inf, false, true)
    ...
    new_upper, new_has_upper,
    new_upper_inclusive = if !vb1.has_upper && !vb2.has_upper
        (Inf, false, true)
...
"""
function extract_term_bounds(conjunctive_form; silent::Bool = true)::Dict{
        Int, VariableBounds,}
    # Get atoms using our safe function
    atoms = get_atoms(conjunctive_form)

    !silent &&
        @info "Processing conjunctive form" n_atoms=length(atoms) type=typeof(conjunctive_form)
...
"""
function strictly_dominates(bounds1::Dict{Int, VariableBounds}, bounds2::Dict{
        Int, VariableBounds,})::Bool
...
function bounds_equal(vb1::VariableBounds, vb2::VariableBounds)::Bool
    return (vb1.has_lower == vb2.has_lower) &&
           ...
           (!vb1.has_lower || (vb1.lower_bound == vb2.lower_bound &&
             vb1.lower_inclusive == vb2.lower_inclusive)) &&
           (!vb1.has_upper || (vb1.upper_bound == vb2.upper_bound &&
...
function refine_dnf(dnf_formula; silent::Bool = true)
    ...
    !silent &&
        @info "Starting DNF minimization" original_terms=n_terms formula_type=typeof(dnf_formula)
