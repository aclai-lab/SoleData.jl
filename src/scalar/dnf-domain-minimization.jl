...
"""
    relation_symbol(relation::Function) -> String

Convert a relation function to its symbolic representation.
"""
function relation_symbol(relation::Function)::String
    relation === (≥) && return "≥"
    relation === (>) && return ">"
    relation === (<) && return "<"
    relation === (≤) && return "≤"
...
"""
    print_dnf(dnf_formula) -> String

Convert a DNF formula to human-readable string representation.
Fixed to handle SoleLogics structures properly.
"""
function print_dnf(dnf_formula)::String
    terms = get_atoms(dnf_formula)
    
    term_strings = map(terms) do term
        atoms = get_atoms(term)
        constraint_strings = map(atoms) do atom
            constraint = extract_constraint(atom; silent=true)
            symbol = relation_symbol(constraint.relation)
            "V$(constraint.variable_id) $symbol $(constraint.threshold)"
        end
...
"""
    format_bounds(vb::VariableBounds, var_id::Int) -> String

Format variable bounds for human-readable output.
"""
function format_bounds(vb::VariableBounds, var_id::Int)::String
    if vb.has_lower && vb.has_upper
        lower_op = vb.lower_inclusive ? "≥" : ">"
        upper_op = vb.upper_inclusive ? "≤" : "<"
        return "$(vb.lower_bound) $lower_op V$var_id $upper_op $(vb.upper_bound)"
    elseif vb.has_lower
        op = vb.lower_inclusive ? "≥" : ">"
        return "V$var_id $op $(vb.lower_bound)"
    elseif vb.has_upper
        op = vb.upper_inclusive ? "≤" : "<"
        return "V$var_id $op $(vb.upper_bound)"
    else
        return "V$var_id: unconstrained"
    end
end

"""
    print_bounds(bounds::Dict{Int,VariableBounds}; prefix::String="")

Print variable bounds in a readable format.
"""
function print_bounds(bounds::Dict{Int,VariableBounds}; prefix::String="")
    if isempty(bounds)
        println(prefix, "No constraints")
        return
    end
    
    sorted_vars = sort(collect(keys(bounds)))
    for var_id in sorted_vars
        println(prefix, format_bounds(bounds[var_id], var_id))
    end
end
...
"""
    debug_domination(bounds1::Dict{Int,VariableBounds}, bounds2::Dict{Int,VariableBounds}, 
                    term1_idx::Int, term2_idx::Int)

Debug utility to check why term1 does/doesn't dominate term2.
"""
function debug_domination(bounds1::Dict{Int,VariableBounds}, bounds2::Dict{Int,VariableBounds}, 
                         term1_idx::Int, term2_idx::Int)
    println("\n=== DOMINATION DEBUG: Term $term1_idx vs Term $term2_idx ===")
    
    all_vars = Set(union(keys(bounds1), keys(bounds2)))
    
    println("Term $term1_idx bounds:")
    print_bounds(bounds1; prefix="  ")
    println("Term $term2_idx bounds:")
    print_bounds(bounds2; prefix="  ")
    
    has_strictly_weaker = false
    all_dominate_or_equal = true
    
    for var_id in sort(collect(all_vars))
        vb1 = get(bounds1, var_id, VariableBounds())
        vb2 = get(bounds2, var_id, VariableBounds())
        
        dominates = strictly_dominates(vb1, vb2)
        equals = bounds_equal(vb1, vb2)
        
        if dominates
            has_strictly_weaker = true
            println("  V$var_id: Term $term1_idx STRICTLY DOMINATES Term $term2_idx")
        elseif equals
            println("  V$var_id: Equal constraints")
        else
            all_dominate_or_equal = false
            println("  V$var_id: Term $term1_idx does NOT dominate Term $term2_idx")
        end
    end
    
    overall_dominates = all_dominate_or_equal && has_strictly_weaker
    println("\nResult: Term $term1_idx $(overall_dominates ? "DOMINATES" : "does NOT dominate") Term $term2_idx")
    println("  All dominate or equal: $all_dominate_or_equal")
    println("  Has strictly weaker: $has_strictly_weaker")
end
...
"""
refine_dnf_and_display(dnf_formula; config=nothing) -> typeof(dnf_formula)

Convenience function that minimizes and displays before/after statistics.
"""
function refine_dnf_and_display(dnf_formula; config=nothing)
    terms = get_atoms(dnf_formula)
    original_count = length(terms)
    
    println("Original formula ($original_count terms):")
    println(print_dnf(dnf_formula))
    println("\n" * "="^70)
    
    minimized = refine_dnf(dnf_formula; silent=false, config)
    minimized_terms = get_atoms(minimized)
    minimized_count = length(minimized_terms)
    
    println("="^70)
    println("\nMinimized formula ($minimized_count terms):")
    println(print_dnf(minimized))
    
    reduction = original_count - minimized_count
    reduction_percent = round((reduction / original_count) * 100, digits=1)
    
    println("\nSummary:")
    println("   Reduction: $original_count → $minimized_count terms (-$reduction)")
    println("   Efficiency: $reduction_percent% reduction achieved")
