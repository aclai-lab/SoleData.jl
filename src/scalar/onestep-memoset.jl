...
@inline function onestep_aggregation(
    ...
    vs = [
        featvalue(f, X, i_instance, w2) for
        w2 in representatives(X, i_instance, w, r, f, aggr)
    ]
    U = featvaltype(X)
...
function apply_aggregator(
    ...
    # TODO remove this SoleData.aggregator_to_binary...

    if length(collect(worlds)) == 0
        aggregator_bottom(aggregator, U)
    else
        aggregator((w)->readfeature(X, featchannel, w, f), worlds)
    end

    # opt = SoleData.aggregator_to_binary(aggregator)
...
"""
abstract type AbstractScalarOneStepRelationalMemoset{
    W<:AbstractWorld,U,FR<:AbstractFrame{W}
} <: AbstractMemoset{W,U,FT where FT<:AbstractFeature,FR} end
...
@inline function Base.getindex(
    ...
    return error(
        "Please, provide method Base.getindex(" *
        "Xm::$(typeof(Xm)), " *
...
@inline function Base.setindex!(
    ...
    return error(
        "Please, provide method Base.setindex!(" *
        "Xm::$(typeof(Xm)), " *
        "gamma, " *
        "i_instance::$(typeof(i_instance)), " *
        "w::$(typeof(w)), " *
...
"""
abstract type AbstractScalarOneStepGlobalMemoset{W<:AbstractWorld,U} <: AbstractMemoset{
    W,U,FT where FT<:AbstractFeature,FR where FR<:AbstractFrame{W}
} end

@inline function Base.getindex(
    Xm::AbstractScalarOneStepGlobalMemoset{W}, i_instance::Integer, i_metacond::Integer
) where {W}
    return error(
        "Please, provide method Base.getindex(" *
        "Xm::$(typeof(Xm)), " *
...
function innerstruct(
    Xm::Union{AbstractScalarOneStepRelationalMemoset,AbstractScalarOneStepGlobalMemoset}
)
    return error("Please, provide method innerstruct(::$(typeof(Xm))).")
end

function isminifiable(
    ::Union{AbstractScalarOneStepRelationalMemoset,AbstractScalarOneStepGlobalMemoset}
)
    true
end
    ...
    Base.@propagate_inbounds function ScalarOneStepMemoset(
        ...
        compute_globmemoset = begin
            if globalrel in relations
                ...
                if W == OneWorld ||
                    all(i_instance->nworlds(frame(X, i_instance)) == 1, 1:ninstances(X))
        ...
        relmemoset = relational_memoset_type(
            X, metaconditions, relations, perform_initialization
        )
        ...
        if (compute_globmemoset && precompute_globmemoset) || precompute_relmemoset
            ...
            if print_progress
                p = Progress(n_instances; dt=1, desc="Computing supports...")
            end
                    ...
                    if compute_globmemoset && precompute_globmemoset
                        for (i_metacond, aggregator, _metacond) in these_metaconditions
                            gamma = featchannel_onestep_aggregation(
                                X, _featchannel, i_instance, globalrel, _feature, aggregator
                            )
                    ...
                    if precompute_relmemoset
                        for (i_relation, relation) in enumerate(relations)
                            for w in allworlds(X, i_instance)
                                for (i_metacond, aggregator, _metacond) in
                                    these_metaconditions
                                    ...
                                    relmemoset[i_instance, w, i_metacond, i_relation] =
                                        gamma
                                end
                            end
                        end
        ...
        ScalarOneStepMemoset{U}(
            relmemoset, globmemoset, metaconditions, relations; silent=silent
        )
    end
end
...
function nmemoizedvalues(Xm::ScalarOneStepMemoset)
    sum(nmemoizedvalues, [relmemoset(Xm), globmemoset(Xm)])
end
...
function grouped_metaconditions(
    ...
    return map(
        ...
        groupbyfeature(metaconditions, features),
    )
end
...
function _featchannel_onestep_aggregation(
    ...
    if isnothing(i_metacond)
        ...
        i_metacond = findfirst(
            (
                m
            )->feature(m) == feature(metacond) &&
               existential_aggregator(test_operator(m)) ==
               existential_aggregator(test_operator(metacond)),
            metaconditions(Xm),
        )
        if isnothing(i_metacond)
            i_neg_metacond = _findfirst(isequal(dual(metacond)), metaconditions(Xm))
            error(
                "Could not find metacondition $(metacond) in memoset of type $(typeof(Xm)) " *
        ...
        if rel == globalrel
            ...
            if isnothing(_globmemoset)
                ...
                if isnothing(_globmemoset[i_instance, i_metacond])
                    gamma = featchannel_onestep_aggregation(
                        X, featchannel, i_instance, rel, _feature, aggregator
                    )
            ...
            i_relation = if isnothing(i_relation)
                _findfirst(isequal(rel), Xm.relations)
            else
                i_relation
            end
            ...
            if isnothing(_relmemoset[i_instance, w, i_metacond, i_relation])
                gamma = featchannel_onestep_aggregation(
                    X, featchannel, i_instance, w, rel, _feature, aggregator
                )
    ...
    ScalarOneStepMemoset{U}(
        ...
        (
            if isnothing(globmemoset(Xm))
                ...
                instances(globmemoset(Xm), inds, return_view)
            end
        ),
        metaconditions(Xm),
        relations(Xm);
...
function concatdatasets(Xms::ScalarOneStepMemoset{U}...) where {U}
    ...
    ScalarOneStepMemoset{U}(
        ...
        (
            if any(isnothing.(globmemoset.(Xms)))
                nothing
            else
...
function isminifiable(Xm::ScalarOneStepMemoset)
    isminifiable(relmemoset(Xm)) &&
        (isnothing(globmemoset(Xm)) || isminifiable(globmemoset(Xm)))
end

function minify(Xm::OSSD) where {OSSD<:ScalarOneStepMemoset}
    (new_relmemoset, new_globmemoset), backmap = minify([relmemoset(Xm), globmemoset(Xm)])

    Xm = OSSD(new_relmemoset, new_globmemoset, featsnaggrs(Xm))
    Xm, backmap
end
...
function displaystructure(
    ...
    padattribute(l, r, off=0) =
        string(l) *
        lpad(string(r), 32+off+length(string(r))-(length(indent_str)+2+length(string(l)))-1)
    pieces = []
    ...
    push!(
        pieces,
        " $(padattribute("metaconditions:", "$(nmetaconditions(Xm)) -> $(SoleLogics.displaysyntaxvector(metaconditions(Xm)))", 1))",
    )
    push!(
        pieces,
        " $(padattribute("relations:", "$(nrelations(Xm)) -> $(SoleLogics.displaysyntaxvector(relations(Xm)))"))",
    )
    ...
    push!(
        ...
        "[R] " * displaystructure(
            ...
            include_nmetaconditions=false,
            include_nrelations=false,
            kwargs...,
        ),
    )
    if !isnothing(globmemoset(Xm))
...
struct ScalarOneStepRelationalMemoset{
    W<:AbstractWorld,U,FR<:AbstractFrame{W},D<:AbstractArray{<:AbstractDict{W,U},3}
...
function instances(
    Xm::ScalarOneStepRelationalMemoset{W,U,FR},
    inds::AbstractVector,
    return_view::Union{Val{true},Val{false}}=Val(false),
) where {W,U,FR}
    ScalarOneStepRelationalMemoset{W,U,FR}(
...
function displaystructure(
    ...
    pieces = []
    push!(
        pieces, "ScalarOneStepRelationalMemoset ($(memoizationinfo(Xm)), $(humansize(Xm)))"
    )
...
struct ScalarOneStepGlobalMemoset{
    W<:AbstractWorld,U,D<:AbstractArray{UU,2} where {UU<:Union{U,Nothing}}
...
@inline function Base.getindex(
    Xm::ScalarOneStepGlobalMemoset{W}, i_instance::Integer, i_metacond::Integer
) where {W<:AbstractWorld}
    Xm.d[i_instance, i_metacond]
end

@inline function Base.setindex!(
    Xm::ScalarOneStepGlobalMemoset{W}, gamma, i_instance::Integer, i_metacond::Integer
) where {W<:AbstractWorld}
