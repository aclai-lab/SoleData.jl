...
@inline function onestep_aggregation(
    ...
    vs = [featvalue(f, X, i_instance, w2)
          for w2 in representatives(X, i_instance, w, r, f, aggr)]
    U = featvaltype(X)
...
abstract type AbstractScalarOneStepGlobalMemoset{W <: AbstractWorld, U} <: AbstractMemoset{
    W, U, FT where FT <: AbstractFeature, FR where FR <: AbstractFrame{W},} end
...
function innerstruct(Xm::Union{
        AbstractScalarOneStepRelationalMemoset, AbstractScalarOneStepGlobalMemoset,})
    return error("Please, provide method innerstruct(::$(typeof(Xm))).")
end

function isminifiable(::Union{
        AbstractScalarOneStepRelationalMemoset, AbstractScalarOneStepGlobalMemoset,})
    true
end

function minify(Xm::Union{
        AbstractScalarOneStepRelationalMemoset, AbstractScalarOneStepGlobalMemoset,})
    minify(innerstruct(Xm))
end

function usesfullmemo(::Union{
        AbstractScalarOneStepRelationalMemoset, AbstractScalarOneStepGlobalMemoset,})
    false
end
                    ...
                    if compute_globmemoset && precompute_globmemoset
                        for (i_metacond, aggregator, _metacond) in these_metaconditions
                            gamma = featchannel_onestep_aggregation(
                                X, _featchannel, i_instance,
                                globalrel, _feature, aggregator,)
                        ...
                        for (i_relation, relation) in enumerate(relations)
                            for w in allworlds(X, i_instance)
                                for (i_metacond, aggregator, _metacond) in
                                    ...
                                    gamma = featchannel_onestep_aggregation(
                                        X, _featchannel, i_instance, w,
                                        relation, _feature, aggregator,)
...
function _featchannel_onestep_aggregation(
    ...
    gamma = begin
        if rel == globalrel
            ...
            i_relation = isnothing(i_relation) ? _findfirst(isequal(rel), Xm.relations) :
                         i_relation
...
function instances(Xm::ScalarOneStepMemoset{U}, inds::AbstractVector,
        return_view::Union{Val{true}, Val{false}} = Val(false),) where {U}
    ScalarOneStepMemoset{U}(
        instances(relmemoset(Xm), inds, return_view),
        (isnothing(globmemoset(Xm)) ? nothing :
...
function concatdatasets(Xms::ScalarOneStepMemoset{U}...) where {U}
    ...
    ScalarOneStepMemoset{U}(
        ...
        (any(isnothing.(globmemoset.(Xms))) ? nothing :
...
function minify(Xm::OSSD) where {OSSD <: ScalarOneStepMemoset}
    (new_relmemoset, new_globmemoset), backmap = minify([
        relmemoset(Xm),
        globmemoset(Xm),
    ])
...
function padattribute(l, r, off = 0)
    string(l) *
...
function displaystructure(
    ...
    push!(pieces,
        "[R] " * displaystructure(
            relmemoset(Xm); indent_str = "$(indent_str)│ ", include_ninstances = false,
            include_nmetaconditions = false, include_nrelations = false, kwargs...,),)
    if !isnothing(globmemoset(Xm))
        push!(pieces,
            "[G] " * displaystructure(globmemoset(Xm); indent_str = "$(indent_str)  ",
                include_ninstances = false, include_nmetaconditions = false, kwargs...,),)
    else
        push!(pieces, "[G] −")
    end
...
struct ScalarOneStepRelationalMemoset{
    ...
    function ScalarOneStepRelationalMemoset{W,
            U,
            FR,}(
            d::D,
    ) where {W <: AbstractWorld, U, FR <: AbstractFrame{W},
            D <: AbstractArray{<:AbstractDict{W, U}, 3},}
        new{W, U, FR, D}(d)
    end
...
function instances(Xm::ScalarOneStepRelationalMemoset{W, U, FR}, inds::AbstractVector,
        return_view::Union{Val{true}, Val{false}} = Val(false),) where {W, U, FR}
    ScalarOneStepRelationalMemoset{W, U, FR}(if return_view == Val(true)
        @view Xm.d[inds, :, :]
    else
        Xm.d[inds, :, :]
    end)
...
struct ScalarOneStepGlobalMemoset{
    ...
    function ScalarOneStepGlobalMemoset{W,
            U,}(
            d::D
    ) where {W <: AbstractWorld, U,
            D <: AbstractArray{UU, 2} where {UU <: Union{U, Nothing}},}
...
function instances(
    ...
    ScalarOneStepGlobalMemoset{W, U}(if return_view == Val(true)
        @view Xm.d[inds, :]
    else
        Xm.d[inds, :]
    end)
