...
function Base.in(item, uv::UniqueVector{<:Union{
        AbstractFeature, AbstractCondition, AbstractRelation,}})
    haskey(uv.lookup, item)
end

function Base.in(item::T,
        uv::UniqueVector{T},) where {T <: Union{
        AbstractFeature, AbstractCondition, AbstractRelation,}}
    haskey(uv.lookup, item)
end

function Base.findfirst(p::UniqueVectors.EqualTo,
        uv::UniqueVector{<:Union{AbstractFeature, AbstractCondition, AbstractRelation}},)
    get(uv.lookup, p.x, nothing)
end

function _in(item, uv::UniqueVector{<:Union{
        AbstractFeature, AbstractCondition, AbstractRelation,}})
    haskey(uv.lookup, item)
end
function _findfirst(p::UniqueVectors.EqualTo,
        uv::UniqueVector{<:Union{AbstractFeature, AbstractCondition, AbstractRelation}},)
    get(uv.lookup, p.x, nothing)
end
...
struct ScalarChainedMemoset{
    ...
    function ScalarChainedMemoset{W,
            U,
            FR,
            D,}(
            d::D
...
function instances(
    ...
    ScalarChainedMemoset(if return_view == Val(true)
        @view Xm.d[inds]
    else
        Xm.d[inds]
    end)
