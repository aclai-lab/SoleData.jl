...
function Base.in(
    item, uv::UniqueVector{<:Union{AbstractFeature,AbstractCondition,AbstractRelation}}
)
    haskey(uv.lookup, item)
end

function Base.in(
    item::T, uv::UniqueVector{T}
) where {T<:Union{AbstractFeature,AbstractCondition,AbstractRelation}}
    haskey(uv.lookup, item)
end

function Base.findfirst(
    p::UniqueVectors.EqualTo,
    uv::UniqueVector{<:Union{AbstractFeature,AbstractCondition,AbstractRelation}},
)
...
struct ScalarChainedMemoset{
    W<:AbstractWorld,U,FR<:AbstractFrame{W},D<:AbstractVector{<:AbstractDict{<:Formula,U}}
...
@inline function Base.haskey(Xm::ScalarChainedMemoset, i_instance::Integer, f::Formula)
    haskey(Xm.d[i_instance], f)
end

@inline function Base.getindex(Xm::ScalarChainedMemoset, i_instance::Integer)
    Xm.d[i_instance]
end
@inline function Base.getindex(Xm::ScalarChainedMemoset, i_instance::Integer, f::Formula)
    Xm.d[i_instance][f]
end
@inline function Base.setindex!(
    Xm::ScalarChainedMemoset, i_instance::Integer, f::Formula, threshold::U
) where {U}
    Xm.d[i_instance][f] = threshold
end

function check(
    f::Formula, i::SoleLogics.LogicalInstance{<:ScalarChainedMemoset{W}}, w::W; kwargs...
