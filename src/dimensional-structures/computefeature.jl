...
function computeunivariatefeature(f::UnivariateFeature{U}, varchannel::Union{
        T, AbstractArray{T},}) where {U, T}
    # (f.f(SoleBase.vectorize(varchannel);))::U
    (f.f(varchannel))::U
end
function computeunivariatefeature(f::UnivariateNamedFeature, varchannel::Union{
        T, AbstractArray{T},}) where {T}
    return error("Cannot intepret UnivariateNamedFeature on any structure at all.")
end
function computeunivariatefeature(f::VariableValue, varchannel::Union{
        T, AbstractArray{T},}) where {T}
