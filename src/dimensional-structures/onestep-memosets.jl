...
struct UniformFullDimensionalOneStepRelationalMemoset{
...
} <: AbstractUniformFullDimensionalOneStepRelationalMemoset{
    U, W, FullDimensionalFrame{N, W},}

    # Multi-dimensional structure
    d::D

    function UniformFullDimensionalOneStepRelationalMemoset{U,
            W,
            N,
            D,}(
            d::D
    ...
    function UniformFullDimensionalOneStepRelationalMemoset(
        ...
        d = begin
            if perform_initialization
                d = Array{Union{U, Nothing}, 5}(undef, size(X, 1), size(X, 2),
                    ninstances(X), nmetaconditions, nrelations,)
                fill!(d, nothing)
            else
                Array{U, 5}(undef, size(X, 1), size(X, 2),
                    ninstances(X), nmetaconditions, nrelations,)
            end
        end
        ...
        d = begin
            if perform_initialization
                d = Array{Union{U, Nothing}, 7}(
                    undef, size(X, 1), size(X, 2), size(X, 3), size(X, 4),
                    ninstances(X), nmetaconditions, nrelations,)
                fill!(d, nothing)
            else
                Array{U, 7}(undef, size(X, 1), size(X, 2), size(X, 3), size(X, 4),
                    ninstances(X), nmetaconditions, nrelations,)
            end
        end
...
function capacity(Xm::UniformFullDimensionalOneStepRelationalMemoset{
        U, <:Interval,}) where {U}
    prod([
        ninstances(Xm),
        nmetaconditions(Xm),
...
function hasnans(Xm::UniformFullDimensionalOneStepRelationalMemoset{
        U, <:Interval,}) where {U}
...
function instances(
    ...
    UniformFullDimensionalOneStepRelationalMemoset{U, W, N}(if return_view == Val(true)
        @view Xm.d[inds, :, :]
    else
        Xm.d[inds, :, :]
    end)
...
function concatdatasets(Xms::UniformFullDimensionalOneStepRelationalMemoset{
        U, W, N,}...) where {U, W <: AbstractWorld, N}
