...
function islogiseed(dataset::AbstractDataFrame)
    true
end

function initlogiset(
    dataset::AbstractDataFrame, features::AbstractVector{<:VarFeature}; kwargs...
)
...
function frame(dataset::AbstractDataFrame, i_instance::Integer; kwargs...)
    column = dataset[:, 1]
...
function featchannel(dataset::AbstractDataFrame, i_instance::Integer, f::AbstractFeature)
    @views dataset[i_instance, :]
end

function readfeature(
    dataset::AbstractDataFrame, featchannel::Any, w::W, f::VarFeature
) where {W<:AbstractWorld}
    _interpret_world(::OneWorld, instance::DataFrameRow) = instance
    _interpret_world(w::Interval, instance::DataFrameRow) = map(
        varchannel->varchannel[w.x:(w.y - 1)], instance
    )
    _interpret_world(w::Interval2D, instance::DataFrameRow) = map(
        varchannel->varchannel[w.x.x:(w.x.y - 1), w.y.x:(w.y.y - 1)], instance
    )
...
function featchannel(
    dataset::AbstractDataFrame, i_instance::Integer, f::AbstractUnivariateFeature
)
...
function _get_wchannel(w::W, featchannel::Any) where {W<:AbstractWorld}
    ...
    _interpret_world(w::Interval2D, varchannel::AbstractArray{T,2}) where {T} = varchannel[
...
function readfeature(
    dataset::AbstractDataFrame, featchannel::Any, w::W, f::AbstractUnivariateFeature
) where {W<:AbstractWorld}
...
function readfeature(
    dataset::AbstractDataFrame, featchannel::Any, w::W, f::VariableDistance
) where {W<:AbstractWorld}
    wchannel = _get_wchannel(w, featchannel)
...
function featvalue(
    feature::AbstractFeature, dataset::AbstractDataFrame, i_instance::Integer, w::W
) where {W<:AbstractWorld}
    readfeature(dataset, featchannel(dataset, i_instance, feature), w, feature)
end

function vareltype(dataset::AbstractDataFrame, i_variable::VariableId)
