...
function frame(
        dataset::AbstractDataFrame,
        column::Vector,
        i_instance::Integer;        # worldtype_by_dim::Union{Nothing,AbstractDict{<:Integer,<:Type}}=nothing
)
    v = column[i_instance]
    if v == ()
        OneWorld()
...
function readfeature(
    ...
    _interpret_world(w::Interval,
        instance::DataFrameRow,) = map(varchannel->varchannel[w.x:(w.y - 1)], instance)
    _interpret_world(w::Interval2D,
        instance::DataFrameRow,) = map(varchannel->varchannel[w.x.x:(w.x.y - 1), w.y.x:(w.y.y - 1)], instance)
