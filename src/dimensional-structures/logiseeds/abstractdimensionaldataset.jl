...
function frame(
    ...
    worldtype = !isnothing(worldtype_by_dim) ? worldtype_by_dim[dimensionality(dataset)] :
                nothing
...
function readfeature(
    ...
    _interpret_world(w::Interval, instance::AbstractArray{
        T, 2,}) where {T} = instance[w.x:(w.y - 1), :]
    _interpret_world(w::Interval2D,
        instance::AbstractArray{T, 3},) where {T} = instance[w.x.x:(w.x.y - 1), w.y.x:(w.y.y - 1), :]
