...
export initlogiset, ninstances, maxchannelsize, worldtype, dimensionality, allworlds,
       featvalue
...
function nrelations(X::SupportedLogiset{
        W, U, FT, FR, L, N, <:Tuple{<:ScalarOneStepMemoset},}) where {W, U, FT, FR, L, N}
    nrelations(supports(X)[1])
end
function nrelations(X::SupportedLogiset{W, U, FT, FR, L, N,
        <:Tuple{<:ScalarOneStepMemoset, <:AbstractFullMemoset},}) where {W, U, FT, FR, L, N}
    nrelations(supports(X)[1])
end
function relations(X::SupportedLogiset{
        W, U, FT, FR, L, N, <:Tuple{<:ScalarOneStepMemoset},}) where {W, U, FT, FR, L, N}
    relations(supports(X)[1])
end
function relations(X::SupportedLogiset{W, U, FT, FR, L, N,
        <:Tuple{<:ScalarOneStepMemoset, <:AbstractFullMemoset},}) where {W, U, FT, FR, L, N}
    relations(supports(X)[1])
end
function nmetaconditions(X::SupportedLogiset{
        W, U, FT, FR, L, N, <:Tuple{<:ScalarOneStepMemoset},}) where {W, U, FT, FR, L, N}
    nmetaconditions(supports(X)[1])
end
function nmetaconditions(X::SupportedLogiset{W, U, FT, FR, L, N,
        <:Tuple{<:ScalarOneStepMemoset, <:AbstractFullMemoset},}) where {W, U, FT, FR, L, N}
    nmetaconditions(supports(X)[1])
end
function metaconditions(X::SupportedLogiset{
        W, U, FT, FR, L, N, <:Tuple{<:ScalarOneStepMemoset},}) where {W, U, FT, FR, L, N}
    metaconditions(supports(X)[1])
end
function metaconditions(X::SupportedLogiset{W, U, FT, FR, L, N,
        <:Tuple{<:ScalarOneStepMemoset, <:AbstractFullMemoset},}) where {W, U, FT, FR, L, N}
    metaconditions(supports(X)[1])
end
