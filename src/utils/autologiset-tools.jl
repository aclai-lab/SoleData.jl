...
AVAILABLE_RELATIONS = OrderedDict{Symbol, Function}([
    ...
    :IA => (d)->[globalrel,
        (d == 1 ? SoleLogics.IARelations :
         (d == 2 ? SoleLogics.IA2DRelations : error("Unexpected dimensionality ($d).")))...,],
    :IA3 => (d)->[globalrel,
        (d == 1 ? SoleLogics.IA3Relations :
         (d == 2 ? SoleLogics.IA32DRelations :
          error("Unexpected dimensionality ($d).")))...,],
    :IA7 => (d)->[globalrel,
        (d == 1 ? SoleLogics.IA7Relations :
         (d == 2 ? SoleLogics.IA72DRelations :
          error("Unexpected dimensionality ($d).")))...,],
...
function defaultrelations(dataset, relations)
    ...
    if dataset isa Union{
        SupportedLogiset{
            W, U, FT, FR, L, N, <:Tuple{<:ScalarOneStepMemoset},} where {
            W, U, FT, FR, L, N,},
        SupportedLogiset{W,
            U,
            FT,
            ...
            L,
            N,
            <:Tuple{<:ScalarOneStepMemoset, <:AbstractFullMemoset},} where {
            W, U, FT, FR, L, N,},
    }
...
function readrelations(model_relations, dataset)
    if model_relations == mlj_default_relations || model_relations isa Symbol
        ...
        if dataset isa Union{
            SupportedLogiset{W, U, FT, FR, L, N,
                <:Tuple{<:ScalarOneStepMemoset},} where {W, U, FT, FR, L, N},
            SupportedLogiset{W,
                U,
                FT,
                FR,
...
function defaultconditions(dataset)
    ...
    elseif dataset isa UniformFullDimensionalLogiset
        vcat([[
                  ...
                  (all(i_instance->SoleData.nworlds(frame(dataset, i_instance)) == 1,
...
function autologiset(
    ...
    var_grouping = begin
        if X isa AbstractDataFrame
            allowedcoltypes = Union{Real, AbstractArray{<:Real, 0},
                AbstractVector{<:Real}, AbstractMatrix{<:Real},}
            wrong_columns = filter(
                ((colname, c),)->!(eltype(c) <: allowedcoltypes), collect(zip(names(X), eachcol(X))),)
                ...
                if isnothing(force_var_grouping)
                    ...
                    if !(length(var_grouping) == 1 && length(var_grouping[1]) == ncol(X))
                        @info "Using variable grouping:\n" *
                              ...
                              join(
                            map(((i_mod, variables),)->"\t{$i_mod} => $(Tuple(variables))",
            ...
            if !passive_mode || !SoleData.ismultilogiseed(X)
                ...
                MultiLogiset([begin
                                  _metaconditions = readconditions(
                                      conditions, featvaltype, mod;
                                      force_i_variables, fixcallablenans,)
...
function _mean(::Type{T1}, vals::AbstractArray{T2}) where {
        T1 <: AbstractFloat, T2 <: Integer,}
    T1(StatsBase.mean(vals))
end
function _mean(::Type{T1}, vals::AbstractArray{T2}) where {
        T1 <: Integer, T2 <: AbstractFloat,}
...
function moving_average(
    ...
    for i_variable in 1:n_variables
        new_instance[:, i_variable] .= [_mean(T, instance[idxs, i_variable])
                                        for idxs in movingwindow(
            npoints; nwindows = nwindows, relative_overlap = relative_overlap,)]
    end
    return new_instance
end
...
function moving_average(
    ...
    for i_variable in 1:n_variables
        new_instance[:, :, i_variable] .= [_mean(T, instance[idxs1, idxs2, i_variable])
                                           for idxs1 in windows_1, idxs2 in windows_2]
    end
    return new_instance
end
