...
struct SupportedLogiset{
    ...
    function SupportedLogiset(
        ...
        wrong_supports = filter(
            supp->!(supp isa Union{<:AbstractVector{<:AbstractDict{<:Formula, <:Worlds}},
                <:Union{AbstractOneStepMemoset, AbstractFullMemoset}, <:SupportedLogiset,}),
            supports,)
        ...
        wrong_supports = filter(
            supp->!(supp isa
                    Union{AbstractOneStepMemoset, AbstractFullMemoset}), supports,)
    ...
    function SupportedLogiset(
            base::AbstractModalLogiset,
            firstsupport::Union{<:AbstractVector{<:AbstractDict{<:Formula, <:Worlds}},
                <:Union{AbstractOneStepMemoset, AbstractFullMemoset}, <:SupportedLogiset,},
            supports::Union{<:AbstractVector{<:AbstractDict{<:Formula, <:Worlds}},
                <:Union{AbstractOneStepMemoset, AbstractFullMemoset},
                <:SupportedLogiset,}...,
    )
        SupportedLogiset(base,
            Union{<:AbstractVector{<:AbstractDict{<:Formula, <:Worlds}},
                <:Union{AbstractOneStepMemoset, AbstractFullMemoset}, <:SupportedLogiset,}[
    ...
    function SupportedLogiset(
        ...
        if use_onestep_memoization != false
            @assert !isnothing(conditions) && !isnothing(relations) "Please, provide " *
                                                                    "both conditions and relations in order to use a one-step memoset."
            onestep_memoset_type = (use_onestep_memoization isa Bool ?
        ...
        if use_full_memoization != false
            full_memoset_type = (use_full_memoization isa Bool ?
                                 default_full_memoset_type(base) : use_full_memoization)
...
function supporttypes(X::SupportedLogiset{
        W, U, FT, FR, L, N, MS,}) where {W, U, FT, FR, L, N, MS}
    MS
end
...
function fullmemo(X::SupportedLogiset)
    usesfullmemo(X) ? last(supports(X)) :
    error("This " *
...
function minify(X::SupportedLogiset)
    (new_sl, new_supports...), backmap = minify([
        base(X),
        supports(X)...,
    ])
...
function concatdatasets(Xs::SupportedLogiset...)
    ...
    SupportedLogiset(
        ...
        [concatdatasets([supports(X)[i_supp] for X in Xs]...)
         for i_supp in 1:nsupports(first(Xs))]...,
    )
end
...
function displaystructure(
    ...
    padattribute(l, r) = string(l) *
                         lpad(r, 32+length(string(r))-(length(indent_str)+2+length(l))-1)
    pieces = []
    push!(pieces,
    ...
    for (i_supp, supp) in enumerate(supports(X))
        push!(pieces,
            "[SUPPORT $(i_supp)] " *
            displaystructure(supp;
                indent_str = (i_supp == nsupports(X) ? "$(indent_str)  " :
                              "$(indent_str)│ "),
                include_ninstances = false,
                include_worldtype = worldtype(X),
                include_featvaltype = featvaltype(X),
