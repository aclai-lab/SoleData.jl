...
If the intent is to check if two AbstractMultiFrameDatasets have same instances regardless of the
...
Two AbstractMultiFrameDatasets are considered "approximately" equivalent if they have same frame
descriptors, attributes and instances.

Note: this means that the order of the instance in the datasets does not matter.

If the intent is to check if two AbstractMultiFrameDatasets have same instances in the same order
...
Two AbstractMultiFrameDatasets are considered similar if they have same frame descriptors and
