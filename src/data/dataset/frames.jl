...
function addframe!(mfd::AbstractMultiFrameDataset, index::Integer)
    @assert index in 1:nattributes(mfd) "Index $(index) is out of range 1:nattributes " *
            "(1:$(nattributes(mfd)))"
  
    push!(descriptor(mfd), [index])
...
function addframe!(mfd::AbstractMultiFrameDataset, attribute_name::Symbol)
    ...
    return addframe!(mfd, _name2index(mfd, attribute_name))
end
"""
...
function removeframe!(mfd::AbstractMultiFrameDataset, indices::AbstractVector{<:Integer})
    for index in indices
        @assert 1 ≤ index ≤ nframes(mfd) "Index $(i) does not correspond to a frame " *
            "(1:$(nframes(mfd)))"
    end

    deleteat!(descriptor(mfd), indices)

    return mfd
end
function removeframe!(mfd::AbstractMultiFrameDataset, index::Integer)
    @assert 1 ≤ index ≤ nframes(mfd) "Index $(index) does not correspond to a frame " *
        "(1:$(nframes(mfd)))"

    deleteat!(descriptor(mfd), index)
...
"""
    dropattribute_fromframe!(mfd, farme_index, attr_indices)    
    dropattribute_fromframe!(mfd, farme_index, attr_index)
    dropattribute_fromframe!(mfd, farme_index, attr_name)
    dropattribute_fromframe!(mfd, farme_index, attr_names)
...
julia> dropattribute_fromframe!(mfd, 3, 5)
...
julia> dropattribute_fromframe!(mfd, 1, :age)
● MultiFrameDataset
    └─ dimensions: (0, 0)
- Frame 1 / 2
    └─ dimension: 0
...
julia> dropattribute_fromframe!(mfd, 2, [3,4])
● MultiFrameDataset
    └─ dimensions: (0, 0)
- Frame 1 / 2
    └─ dimension: 0
...
julia> dropattribute_fromframe!(mfd, 1, [:name,:height])
...
─────┼──────────────────────────────
    ...
    2 │ Julia   F        175      60    
"""
function dropattribute_fromframe!(
    mfd::AbstractMultiFrameDataset, frame_index::Integer, attr_indices::AbstractVector{<:Integer}
)
    for attr_index in attr_indices
        dropattribute_fromframe!(mfd, frame_index, attr_index)
    end
...
function dropattribute_fromframe!(
    ...
    for attr_name in attr_names
        dropattribute_fromframe!(mfd, frame_index, _name2index(mfd, attr_name))
    end
...
function dropattribute_fromframe!(
    ...
    return dropattribute_fromframe!(mfd, frame_index, _name2index(mfd, attr_name))
end
...
function dropframe!(mfd::AbstractMultiFrameDataset, indices::AbstractVector{<:Integer})
    for (i, index) in enumerate(indices)
        dropattributes!(mfd, descriptor(mfd)[index])
        
        if any(x->x>index, indices[i:length(indices)])
            indices[findall(indices[i:length(indices)] .> index)] .-= 1
        end
    end
...
function dropframe!(mfd::AbstractMultiFrameDataset, index::Integer)
    ...
    return dropattributes!(mfd, descriptor(mfd)[index])
end
