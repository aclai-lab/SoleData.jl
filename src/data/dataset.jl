...
function describeonm(df::AbstractDataFrame; kwargs...)
    cols = findall([eltype(c) <: AbstractVector{<:Number} for c in eachcol(df)])
    
    results1 = Array{Float64}[]
    results2 = Array{Float64}[]
    results3 = Array{Float64}[]
    for j in 1:length(cols)
        result_mean = []
        result_min = []
        result_max = []
        for i in 1:nrow(df)
            push!(result_mean, round(mean(df[!, cols[j]][i]); digits=1))
            push!(result_min, minimum(df[!, cols[j]][i]))
            push!(result_max, maximum(df[!, cols[j]][i]))
        end
        
        push!(results1, result_mean)
        push!(results2, result_min)
        push!(results3, result_max)

        # Save result_mean in Dictionary, then push Dictionary into dataframe generated by describe
    end
    #println(resultss)
    df_final = insertcols!(describe(df), ncol(describe(df))+1, :mean_m => [results1[i] for i in 1:length(cols)], 
                                                                :min_m => [results2[i] for i in 1:length(cols)],
                                                                :max_m => [results3[i] for i in 1:length(cols)])
    return df_final
end
...
function DF.describe(mfd::MultiFrameDataset; kwargs...)
    results = DataFrame[]
    for frame in mfd
        push!(results, describeonm(frame; kwargs...))
    end
    return results
end
